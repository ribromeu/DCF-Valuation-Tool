...
This produces a **13×13 grid** (169 cells). Row and column headers show the shock relative to the base case (`−3%`, `base`, `+5%`), not absolute rates — readable regardless of what the base inputs are.  
...
I built this during my BS Economics program (minors in Finance and Data Analysis) as a way to apply corporate finance theory, discounted cash flow modeling, and software engineering to a real valuation problem. DCF is one of the most foundational frameworks in equity research — every CFA candidate learns it, every sell-side analyst uses it — but the raw output creates false precision: a single implied price with no indication of how sensitive it is to the assumptions underneath it.
...
**This tool is for personal use.** I use it to run quick valuations on stocks I'm researching, stress-test my own assumptions before forming a position view, and practice the kind of structured financial modeling I'll be doing professionally in quantitative finance, asset research, or risk management. It is not investment advice and makes no claim to production-grade accuracy.
